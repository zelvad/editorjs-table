...
export class Table {
  ...
  _hangEvents() {
    ...
    this._table.addEventListener("contextmenu", (event) => {
      if (
        ...
        event.target.closest("td,th").className.includes("selected")
      ) {
    ...
    const handleMouseDownOnCell = (event) => {
      if (event.target.closest("td,th")) {
        ...
        if (startRowIndex === targetRowIndex) {
          ...
          if (
            Array.from(table.rows[startRowIndex].cells).some(
  ...
  _showCustomContextMenuOnSelectedCells(event) {
    ...
    changeCellBackgroundColorButton.addEventListener(
      "click",
      this._changeCellBackgroundColor.bind(this)
    )
